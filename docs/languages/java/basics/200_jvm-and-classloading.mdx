...
title="いつ初期化されるか"
options={[
{id: 'a', label: 'ソースに書いた時点で、起動直後に必ず static ブロックが走る'},
{id: 'b', label: 'クラスが初めて必要になり、初期化されるタイミングで走る'},
{id: 'c', label: 'javac の時点で static ブロックは実行される'},
]}
answer="b"
explanation="クラスは必要になったときにロード・初期化されます。static 初期化子は起動直後に必ず走るわけではありません。"

>

  <div>次のブロックについて、正しいものはどれですか。</div>
  <pre className="language-java"><code>{`class PricingRules {
    static {
        System.out.println("PricingRules initialized");
    }
}`}</code></pre>
...
>
  ...
  <div>次のような例外を見るとき、正しい理解はどれですか。</div>
  <pre className="language-text"><code>{`java.lang.ClassCastException: com.example.Shipment
 cannot be cast to com.example.Shipment`}</code></pre>
</QuizChoice>
