...
[型とメモリ](./types-and-memory) で見た「参照は行き先」が、文字列比較ではいちばん効いてきます。  
このページでは、`String` の扱いと、`==` / `equals` の違いを先に固めます。  
`hashCode` と Map / Set の契約は、同値比較のあとに「次の段」として触れます。
...
[コレクションとジェネリクス](./collections-generics) のあとでも読み返せます。  
自作クラスを `HashMap` のキーや `HashSet` の要素にするときだけ、ここが必須になります。
...
コレクション API の使い分けは [コレクションとジェネリクス](./collections-generics) で扱います。ここでは **桶 → equals** の契約だけ見てください。
...
書き方の詳細や現代的な型としての位置づけは、後の [ジェネリクスと現代的な型](./generics-and-modern-types) でも扱います。今は「手書きの契約を生成に任せられる」と分かれば足ります。
