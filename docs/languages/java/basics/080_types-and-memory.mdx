...
[文字列の基本](./string-basics) では、`String` を参照型と呼び、中身の比較に `equals` を使いました。  
ここではその理由を、変数の箱に何が入っているかの地図として回収します。
...
だから `String` の中身比較は `equals` です。`==` は「同じ行き先か」を見る演算子で、文字の並びが同じかは保証しません。  
`==` と `equals` の切り分けは、次の [String と equals](./string-equals) で厚くします。
...
[文字列の基本](./string-basics) では、`null` を「中身が無い」と覚えました。  
地図で言うと、それは **住所欄が空——行き先がない——** という意味です。
