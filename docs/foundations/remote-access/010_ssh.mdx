...
サーバのログやプロセスを調べたくても、そのサーバが手元にあるとは限りません。  
クラウドや別の拠点にある Linux へ、安全な通信経路で入る必要があります。

そこで使うのが **SSH**（Secure Shell）です。  
SSH でリモートのシェルへログインすれば、あとは [Linux / Shell](../linux/) で学んだ操作の延長として調査や設定ができます。
